...
-- Subring intersection using the method from Stillman and Tsai
-- This algorithm is guaranteed (?) to terminate if the
-- given rings are monomial subrings of a polynomial ring
-- 
subringIntersection1 = method(
    Options => {
	Strategy => "Master",
	SubductionMethod => "Top",
	"SAGBILimitType" => "Fixed", -- "Fixed" or "Function"
...
subringIntersection1(Subring, Subring) := opts -> (S1, S2) -> (
    local limit;
    
    -- Write S1 = k[f_1 .. f_s] and S2 = k[g_1 .. g_t]
    -- subrings of k[x_1 .. x_n]/I
    -------------
    -- check that both subring are subrings of the same ambient ring
    Q1 := ambient S1;
    Q2 := ambient S2;
    ...
    -------------
    -- Construct the ring T = k[x_1 .. x_n, y_1 .. y_t] / I + (y_i - g_i) -- write T = TAmb / TIdeal
    -- where S2 = k[g_1 .. g_t]
    -- T has the elimination order on x_i's
    ...
    -- simply use GRevLex + Elimination order 
    -- Warning: forget about the term order on R
    TAmb := C[Variables => n+t, MonomialOrder => Eliminate(n)];
    RintoTAmb := map(TAmb, R, (gens TAmb)_{0 .. n-1});
    TAmbI := RintoTAmb I; 
    
    QtoR := map(R, Q, gens R); -- lifting map: TODO: check if this needs to be compatible with the term order of R and TAmb
    S1GensInTAmb := RintoTAmb QtoR gens S1;
    S2GensInTAmb := RintoTAmb QtoR gens S2;
    
    TBinomialIdeal := ideal(S2GensInTAmb - matrix {(gens TAmb)_{n .. n+t-1}});
    TIdeal := TAmbI + TBinomialIdeal;
    
    T := TAmb / TIdeal;
    TQuotMap := map(T, TAmb, gens T);
    TtoQ := map(Q, T, matrix {gens Q} | gens S2);
    
    -- Subring S = k[f_1 .. f_s] inside of T
    S := subring TQuotMap S1GensInTAmb;
	...
	"ambientRing" => Q,
	"generators" => intersectionGens,
	"subductionQuotientRing" => C[Variables => numcols intersectionGens],
	cache => new CacheTable from {},
	"originalSubrings" => {S1, S2},
	"compositeSubring" => S
	}; 
    -- if we successfully computed a sagbi basis for the composite subring then all good!
    -- the generators of this subring is a sagbi basis?? (seems like its not the case)
    -- NB we have switched term orders to GRevLex so this NEEDS to be fixed ... later.
    -- if isSAGBI SB then forceSB result;
    result
    );
...
------------------
-- Lead term of a subring is generated by the lead terms of the
-- sagbi basis elements
-- Caution this is mostly for the polyomial (in the quotient case something else 
-- should happen i.e. work over the ring R/in(I) instead!)
leadTerm(Subring) := S -> (
    assert(zero ideal ambient S); -- assert not a quotient ring
    SB := subalgebraBasis S;
    leadTerm SB
    ) 
...
------------------------
-- hilbertPolynomial
-- if a subring has a computed SAGBI basis then we can write down the
-- hilbertPolynomial as the Ehrhart polynomial of the NOBody 
...
hilbertPolynomial(Subring) := opts -> S -> (
    ...
    computedNOBody := convexHull(NOBodyVertices);
    --print vertices NOBody;
    ehrhart computedNOBody    
    )

-- Case 2.
-- Graded by the variable at index i 
-- in the ambient ring of S
...
-- TODO:
-- if no sagbi basis is known, use the resolution of the
-- presentation ideal of the subring (taking care of degrees)
--
-- Warning: the subductionQuotientRing should may not have the
-- the correct degrees set
...
NObody(Subring) := S -> (
    ...
    -- project the vertices down to the lower-dimensional lattice
    NOBodyVertices := transpose compress transpose (basisLattice^(-1)*V);    
    convexHull(NOBodyVertices)
    )

-- Case 2.
-- Graded by the variable at index i 
-- in the ambient ring of S
NObody(Subring, ZZ) := (S, i) -> (
    M := leadTerm S;
    exponentMatrix := transpose matrix flatten (first entries M / exponents);
    columnSums := for column from 0 to (numgens source exponentMatrix)-1 list 1/exponentMatrix_(i, column);
    V := exponentMatrix*diagonalMatrix(columnSums);
