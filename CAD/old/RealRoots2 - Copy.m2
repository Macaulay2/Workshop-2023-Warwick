...
realRootIsolation (RingElement,A) := List => (f,r)->(
	...
	--if size(C)=0,M=1. If size(C)=1, M=1+abs(C_0), else do the below (take the 0 out).
...
document {
    	Key => {"Multiplicity(RealRoots2)", [SylvesterCount, Multiplicity], [SturmCount, Multiplicity]},
...
restart
debug needsPackage "RealRoots2"
needsPackage "RealRoots2"
check "RealRoots2"

uninstallPackage "RealRoots2"
restart
